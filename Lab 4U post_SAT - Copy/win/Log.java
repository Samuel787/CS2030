public class Log {
  ...
  public void logWaits(IServer serv, ICustomer cust, double time) {
    if(serv instanceof HumanServer){
      System.out.println(String.format("%.3f", time) + " " + cust.getCustID() + " waits for Human Server: " + serv.getID() +" and it has current queue length of: "+ serv.getCurrQueueLength());
    } else {
      System.out.println(String.format("%.3f", time) + " " + cust.getCustID() + " waits for Machine Server: " + serv.getID() + " and it has current queue length of: "+ serv.getCurrQueueLength());
    }
  }
  public void logServed(IServer serv, ICustomer cust, double time) {
    if(serv instanceof HumanServer){
      System.out.println(String.format("%.3f", time) + " " + cust.getCustID() + " served by Human Server" + serv.getID());
    } else {
      System.out.println(String.format("%.3f", time) + " " + cust.getCustID() + " served by Machine server" + serv.getID());
    }
    this.served++;
  }
