...
public class Simulator {
  ...
  public static String simulate(int[] custIDs, double[] custArriveTime, int numArrival, int servMaxCust) {
    /** Variables */
    // Server
    Queue1 queue1 = new Queue1(1); //maximum 1 people in the queue
    ...
    while(numEvents > 0) {
      ...
      switch(currEvent.getState()) {
        ...
        case SERVED:
          /*
          if(servers.get(currEvent.getServerID()).getCustQueue1().getQueueLength() >= servMaxCust-1){
            servers.get(currEvent.getServerID()).setServerBusy(true);
          }
          */
          /*
          if(servers.get(currEvent.getServerID()).getCustQueue1().isQueueFull()){
            servers.get(currEvent.getServerID()).setServerBusy(true);
          }
          */
