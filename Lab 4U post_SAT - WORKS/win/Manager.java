...
public class Manager{
    ...
    public IServer findFirstFreeServer(){
        for(int i=0; i<servers.size(); i++){
            IServer currentServer = servers.get(i);
            if(currentServer instanceof HumanServer){
                HumanServer currentHumanServer = (HumanServer)currentServer;
                if(!currentHumanServer.isBusy() && !currentHumanServer.getIsResting() && !currentHumanServer.getAboutToRest())
                ...
                if(!currentMachineServer.isBusy())
                    return servers.get(i); //this server is free
            }
        }
    ...
    public IServer findServerForGeneralCustomer(){
        ...
        */
        ...
        //throw in all servers with queue space inside nonFull Servers
        ArrayList<IServer> nonFullServers = new ArrayList<>();
        for(IServer s : servers) if(!s.isQueueFull()) nonFullServers.add(s);
        
        if(nonFullServers.size() == 0) return null;
        
        //assuming there are no free servers
        int shortest_length = nonFullServers.get(0).getCurrQueueLength();
        for(int i=1; i< nonFullServers.size(); i++){
            if(nonFullServers.get(i).getCurrQueueLength() < shortest_length){
                shortest_length = nonFullServers.get(i).getCurrQueueLength();
            }
        }

        //get all the queues with shortest_length
        ArrayList<IServer> shortestServers = new ArrayList<>();
        for(IServer i: nonFullServers){
